...
### اصطلاحات و انطباق آن‌ها با این توپولوژی
...
آدرس‌های `.16` و `.31` در بلوک `/28` قابل واگذاری به میزبان نیستند. بازه معتبر آن بلوک `.17` تا `.30` است:
...
- ping مستقیم `192.168.0.2` نیز ممکن است از default route عبور کند و به دلیل source NAT روی R5 پاسخ‌پذیر شود، اما در `Internet` ترجمه مقصد static را طی نمی‌کند؛
...
| `PC1 -> 192.168.0.2` | ترجمه source از `10.10.6.1` به pool | نگاشت static موجود است، ولی مقصد اولیه `.2` است و ترجمه destination public-to-local لازم نیست |
...
4. در `Simulation` مسیر را دنبال کنید:
   ...
   - `Internet` مقصد را روی شبکه متصل `213.80.11.0/24` می‌بیند؛
   - `R5` برای global static پاسخ ARP می‌دهد و مقصد را از `213.80.11.32` به `10.10.7.2` تبدیل می‌کند؛
   - پاسخ Server1 در مسیر برگشت، source را از `10.10.7.2` به `213.80.11.32` تبدیل می‌کند.
...
1. فقط فایل‌های مربوط به آزمایش ششم و شواهد لازم را در بسته قرار دهید.
2. فایل Packet Tracer نهایی را قبل از ZIP باز کنید و از سالم‌بودن آن مطمئن شوید.
3. ساختار پیشنهادی بسته:
...
4. پوشه را با نام زیر ZIP کنید:

```text
KiarashShojaei-6-2-6.zip
```

5. نام رسمی کلی سند `FullName-GroupNum-ClassNum-ExperimentNum.zip` است.
